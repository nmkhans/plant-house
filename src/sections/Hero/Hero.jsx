    ...
    <section className="flex flex-col-reverse lg:flex-row justify-between items-center my-5">
      <div className="basis-full lg:basis-1/4 w-full">
        <div className="dropdown dropdown-open w-full">
          <div className="bg-primary text-white shadow-md mb-3 p-3 rounded">
            Categories
          </div>
            ...
            {categories?.slice(query.start, query.end).map((category) => (
              <li key={category._id} className="border">
                <Link to={`/category/${category.name}`}>
                  {capitalize(category.name)} Plant
                </Link>
              </li>
            ))}
          </ul>
            ...
            {slides.map((slide) => (
              <div key={slide.title} className="relative h-[200px] lg:h-auto">
                ...
                <div className="absolute top-[15%] lg:top-[30%] left-[5%] text-left">
                  <h3 className="text-primary text-xl lg:text-4xl font-semibold mb-2 lg:mb-5">
                    {slide.title}
                  </h3>
                  <p className="text-[12px] lg:text-[16px] text-base-200 w-2/3 mb-3">
                    {slide.description}
                  </p>
                  <button
